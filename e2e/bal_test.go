...
func TestE2E_BAL_CrossNodeAgreement(t *testing.T) {
	...
	cluster := frameworkV2.NewTestCluster(t, 4,
		...
		frameworkV2.WithBALNonValidators(),
		frameworkV2.WithPremine(map[types.Address]*big.Int{sender.Address(): ethgo.Ether(100)}),
	)
	defer cluster.Stop()
	...
	assertCodeParity(t, cluster, types.Address(contractAddr))
	...
	assertStorageParity(t, cluster, types.Address(contractAddr), types.Hash{}, newVal)

	// 4. Negative: sender balance parity after all the above
...
func TestE2E_BAL_RevertedTx_NoStatePersisted(t *testing.T) {
	...
	assertStorageParity(t, cluster, types.Address(contractAddr), types.Hash{}, big.NewInt(0))
}
...
func TestE2E_BAL_NestedCall_Success(t *testing.T) {
	...
	cluster := frameworkV2.NewTestCluster(t, 6,
		frameworkV2.WithNonValidators(2),
		frameworkV2.WithBootnodeCount(1),
		frameworkV2.WithBALNonValidators(),
		frameworkV2.WithPremine(map[types.Address]*big.Int{
			sender.Address(): ethgo.Ether(100),
		}),
	)
	defer cluster.Stop()
