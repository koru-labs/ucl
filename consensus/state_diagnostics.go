...
//
// Count is the number of accepted messages of the type. For "prepare" the
// proposer's PREPREPARE counts towards the quorum without a PREPARE message;
// ProposerImplied reports when ReceivedPower includes that implied vote, so
// Count and ReceivedPower may differ by one validator.
type QuorumInfo struct {
	Available       bool   `json:"available"`
	Count           int    `json:"count"`
	ProposerImplied bool   `json:"proposerImplied,omitempty"`
