...
type PathGenerator func(addr types.Address, i int, j int, total int) Key
...
type Sender func(int) types.Address

func NewTestExecTask(txIdx int, ops []Op, sender types.Address, nonce int) *testExecTask {
	return &testExecTask{
		txIdx:        txIdx,
		ops:          ops,
...
func (t *testExecTask) Sender() types.Address {
	return t.sender
}

func (t *testExecTask) Hash() types.Hash {
	return types.BytesToHash([]byte(fmt.Sprintf("%d", t.txIdx)))
}
...
var randomPathGenerator = func(sender types.Address, i int, j int, total int) Key {
	return NewStateKey(
		types.BytesToAddress(big.NewInt(int64(i%10)).Bytes()),
		types.Hash(types.BytesToHash(big.NewInt(int64(total)).Bytes())))
}

var dexPathGenerator = func(sender types.Address, i int, j int, total int) Key {
	if j == total-1 || j == 2 {
		return NewSubpathKey(types.BytesToAddress(big.NewInt(int64(0)).Bytes()), 1)
	} else {
		return NewSubpathKey(types.BytesToAddress(big.NewInt(int64(j)).Bytes()), 1)
	}
}
...
func taskFactory(numTask int, sender Sender, readsPerT int, writesPerT int, nonIOPerT int, pathGenerator PathGenerator, readTime Timer, writeTime Timer, nonIOTime Timer) ([]ExecTask, time.Duration) {
	...
	senderNonces := make(map[types.Address]int)
		...
		for _, numRead := range numReads {
			for _, numWrite := range numWrites {
				for _, numNonIO := range numNonIO {
					fmt.Println("Executing block", "numTx", numTx, "numRead", numRead, "numWrite", numWrite, "numNonIO", numNonIO)
...
func TestLessConflicts(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address {
			randomness := rand.Intn(10) + 10
			return types.BytesToAddress(big.NewInt(int64(i % randomness)).Bytes())
		}
...
func TestZeroTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address { return types.BytesToAddress(big.NewInt(int64(1)).Bytes()) }
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestAlternatingTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address { return types.BytesToAddress(big.NewInt(int64(i % 2)).Bytes()) }
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestRandomTx(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		...
		sender := func(i int) types.Address { return types.BytesToAddress(big.NewInt(int64(rand.Intn(10))).Bytes()) }
		tasks, serialDuration := taskFactory(numTx, sender, numRead, numWrite, numNonIO, randomPathGenerator, readTime, writeTime, nonIOTime)
...
func TestDexScenario(t *testing.T) {
	...
	taskRunner := func(numTx int, numRead int, numWrite int, numNonIO int) (time.Duration, time.Duration) {
		sender := func(i int) types.Address { return types.BytesToAddress(big.NewInt(int64(i)).Bytes()) }
