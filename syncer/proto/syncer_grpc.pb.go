...
type SyncPeerClient interface {
	...
	GetStatus(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (*SyncPeerStatus, error)
	// Returns stream of transactions from the tx pool
	GetTxPool(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (SyncPeer_GetTxPoolClient, error)
}
...
func (c *syncPeerClient) GetStatus(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (*SyncPeerStatus, error) {
...
func (c *syncPeerClient) GetTxPool(ctx context.Context, in *empty.Empty, opts ...grpc.CallOption) (SyncPeer_GetTxPoolClient, error) {
	stream, err := c.cc.NewStream(ctx, &SyncPeer_ServiceDesc.Streams[1], "/v1.SyncPeer/GetTxPool", opts...)
	if err != nil {
		return nil, err
	}
	x := &syncPeerGetTxPoolClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type SyncPeer_GetTxPoolClient interface {
	Recv() (*Transactions, error)
	grpc.ClientStream
}

type syncPeerGetTxPoolClient struct {
	grpc.ClientStream
}

func (x *syncPeerGetTxPoolClient) Recv() (*Transactions, error) {
	m := new(Transactions)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type SyncPeerServer interface {
	...
	// Returns server's status
	GetStatus(context.Context, *empty.Empty) (*SyncPeerStatus, error)
	// Returns stream of transactions from the tx pool
	GetTxPool(*empty.Empty, SyncPeer_GetTxPoolServer) error
...
func (UnimplementedSyncPeerServer) GetStatus(context.Context, *empty.Empty) (*SyncPeerStatus, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetStatus not implemented")
}
func (UnimplementedSyncPeerServer) GetTxPool(*empty.Empty, SyncPeer_GetTxPoolServer) error {
	return status.Errorf(codes.Unimplemented, "method GetTxPool not implemented")
}
...
func _SyncPeer_GetStatus_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(SyncPeerServer).GetStatus(ctx, req.(*empty.Empty))
	}
	return interceptor(ctx, in, info, handler)
}

func _SyncPeer_GetTxPool_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(empty.Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(SyncPeerServer).GetTxPool(m, &syncPeerGetTxPoolServer{stream})
}

type SyncPeer_GetTxPoolServer interface {
	Send(*Transactions) error
	grpc.ServerStream
}

type syncPeerGetTxPoolServer struct {
	grpc.ServerStream
}

func (x *syncPeerGetTxPoolServer) Send(m *Transactions) error {
	return x.ServerStream.SendMsg(m)
}
...
var SyncPeer_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "GetTxPool",
			Handler:       _SyncPeer_GetTxPool_Handler,
			ServerStreams: true,
		},
	},
