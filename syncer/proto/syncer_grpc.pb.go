...
func (c *syncPeerClient) GetTxPool(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Transactions], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &SyncPeer_ServiceDesc.Streams[1], SyncPeer_GetTxPool_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[emptypb.Empty, Transactions]{ClientStream: stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
func _SyncPeer_GetTxPool_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(SyncPeerServer).GetTxPool(m, &grpc.GenericServerStream[emptypb.Empty, Transactions]{ServerStream: stream})
}
