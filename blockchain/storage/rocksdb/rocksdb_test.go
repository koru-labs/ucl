...
func newStorage(t *testing.T) (*storage.Storage, func(), string) {
	...
	path, err := os.MkdirTemp("", "rocksdbV2")
	if err != nil {
		t.Fatal(err)
	}

	s, err := NewRocksDBStorage(path, hclog.NewNullLogger())
	if err != nil {
		t.Fatal(err)
	}
...
func writeBlock(t *testing.T, s *storage.Storage, b *types.FullBlock) {
	t.Helper()
...
func readBlock(t *testing.T, s *storage.Storage, blockCount int, wg *sync.WaitGroup, ctx context.Context) {
	t.Helper()
...
func TestWriteReadFullBlockInParallel(t *testing.T) {
	...
	go storage.GenerateBlocks(t, blockCount, blockchain, ctx)
