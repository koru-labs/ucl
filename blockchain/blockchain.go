...
func (b *Blockchain) verifyBlockBody(block *types.Block) ([]*types.Receipt, error) {
	...
	if forks.EIP7928 && block.Header.BlockAccessRecordHash != block.BlockAccessRecord.Hash() {
		return nil, fmt.Errorf(
			"block %d block access record hash missmatch")
	}
...
func (b *Blockchain) AddBlockAccessListToCache(hash types.Hash, accessList bal.BlockAccessListEncoded) {
	b.blockAccessRecordCache.Add(hash, accessList)
}
...
func (b *Blockchain) writeBody(batchWriter *storage.Writer, block *types.Block) error {
	...
	batchWriter.PutBody(block.Number(), block.Header.Hash, block.Body())
...
func (b *Blockchain) writeHeaderImpl(
	batchWriter *storage.Writer, evnt *Event, header *types.Header) (bool, *big.Int, error) {
...
func (b *Blockchain) CalculateBaseFee(parent *types.Header) uint64 {
	...
	if !b.config.Params.Forks.IsActive(chain.London, parent.Number+1) {
		return 0
	}

	// Check if this is the first London hardfork block.
	// Should return chain.GenesisBaseFee ins this case.
	if parent.BaseFee == 0 && !b.config.Params.Forks.IsActive(chain.London, parent.Number) {
...
func (b *Blockchain) verifyBlockAccessList(block *types.Block, computedBAL bal.BlockAccessListEncoded) error {
	...
	b.logger.Error("COMPUTED HASH IS:", computedHash, "BLOCK HASH IS:", block.Header.BlockAccessRecordHash)

	if computedHash != block.Header.BlockAccessRecordHash {
