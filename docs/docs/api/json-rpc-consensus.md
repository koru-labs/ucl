...
```json
{
  ...
  "current": {
    ...
    "quorumSize": "3",
    "quorum": { "prepare": { "available": true, "count": 2, "receivedPower": "3", "requiredPower": "3", "hasQuorum": true } },
...
- `quorum.<type>`: `available`, `count`, `receivedPower`, `requiredPower`, `hasQuorum`
...
| `--consensus-state-push-token` / `consensus_state_push_token` | empty | Bearer token required by the ingest API (required when URL is set) |
...
- Treat the push token like a secret; rotate by updating node config + backend `INGEST_TOKEN` and restarting.
