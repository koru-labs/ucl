...
| `--json-rpc-batch-cost-limit`    | Max total method-cost weight of a JSON-RPC batch request. Zero disables the budget.                                                         | `--json-rpc-batch-cost-limit 200`          |
| `--json-rpc-max-response-size`   | Max serialized JSON-RPC response size in bytes. Zero disables the cap.                                                                      | `--json-rpc-max-response-size 26214400`    |
| `--rpc-gas-cap`                  | Maximum gas eth_call / eth_estimateGas / debug_traceCall may use. Zero disables the cap.                                                    | `--rpc-gas-cap 50000000`                   |
...
| `--json-rpc-filter-limit`        | Max number of active filters and subscriptions on the node, across all clients. Zero means no limit.                                        | `--json-rpc-filter-limit 10000`            |
| `--json-rpc-filter-limit-per-connection` | Max number of active subscriptions a single websocket connection may hold. Zero means no limit.                                     | `--json-rpc-filter-limit-per-connection 100` |
| `--json-rpc-ws-max-connections`  | Max number of active websocket connections. Zero means no limit.                                                                    | `--json-rpc-ws-max-connections 1024`       |
| `--json-rpc-ws-max-in-flight`    | Max number of in-flight websocket JSON-RPC requests across all connections. Zero means no limit.                                    | `--json-rpc-ws-max-in-flight 256`          |
| `--json-rpc-ws-max-in-flight-per-connection` | Max number of in-flight websocket JSON-RPC requests per connection. Zero means no limit.                                | `--json-rpc-ws-max-in-flight-per-connection 16` |
